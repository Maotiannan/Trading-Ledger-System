...
- NAS-only 完整备份方案已完成代码实现：数据库和全部上传文件会在 NAS 备份目录生成可校验的原子快照，成功后按 30 天保留；正式定时任务将在真实快照与隔离恢复通过后切换
...
- `Orders` 页面已准备接收 MU Contract 正式 PI 的创建日期和金额；人工订单优先，来源订单改名按隐藏 PI 身份跟随，未匹配记录只对管理员可见
- MU Contract Orders 同步采用首次全量对账预览确认、增量游标、事件幂等、数据库租约和冲突记录；不会写入发票、收据、付款明细、SWIFT、余额或 NAS 文件
...
### 0. 最新里程碑
- MU Contract -> MULEDGER Orders 双端代码和源端部署已完成；MULEDGER 端等待最新 NAS 快照、隔离迁移演练和受控部署，首次全量对账仍需预览确认后才能启用
- NAS-only 备份脚本、校验、30 天保留和定时任务安装器已完成，等待 PR/CI 后创建第一份正式快照并做隔离恢复
