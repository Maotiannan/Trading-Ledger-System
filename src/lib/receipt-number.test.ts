...
describe('receipt-number', () => {
  it('formats receipt numbers as seven digits', () => {
    expect(formatReceiptNo(10000)).toBe('0010000');
    expect(formatReceiptNo(10001)).toBe('0010001');
  });

  it('allocates receipt numbers atomically starting from 0010000', async () => {
  ...
  it('suggests the next receipt number from the atomic counter and skips existing numbers', async () => {
    ...
    await expect(getSuggestedNextReceiptNo(tx)).resolves.toBe('0010001');
    expect(findFirst).toHaveBeenNthCalledWith(1, {
      where: { receiptNo: '0010000' },
      select: { id: true },
    });
    expect(findFirst).toHaveBeenNthCalledWith(2, {
      where: { receiptNo: '0010001' },
      select: { id: true },
    });
  });
  ...
  it('skips occupied receipt numbers while allocating from the atomic counter', async () => {
    ...
    await expect(allocateNextReceiptNo(tx)).resolves.toBe('0010001');
  });

  it('uses a requested seven-digit receipt number and moves the counter past it', async () => {
    ...
    await expect(allocateNextReceiptNo(tx, { requestedReceiptNo: '10005' })).resolves.toBe('0010005');
    expect(findFirst).toHaveBeenCalledWith({
      where: { receiptNo: '0010005' },
      select: { id: true },
    });
  ...
  it('rejects a requested receipt number that already exists', async () => {
    ...
    await expect(allocateNextReceiptNo(tx, { requestedReceiptNo: '0010001' })).rejects.toThrow('收据号 0010001 已存在，请换一个编号');
  });
});
