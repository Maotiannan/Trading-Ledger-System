...
function resolveLogoDataUri() {
  ...
  try {
    ...
    cachedLogoDataUri = `data:image/png;base64,${image.toString('base64')}`;
  } catch {
    cachedLogoDataUri = '';
  }
...
function resolveFontDataUri() {
  ...
  const fontPath = path.join(process.cwd(), 'public', 'detail-export', 'noto-sans-latin-regular.ttf');
  try {
    const font = fs.readFileSync(fontPath);
    cachedFontDataUri = `data:font/ttf;base64,${font.toString('base64')}`;
  } catch {
    cachedFontDataUri = '';
  }
...
function buildTypeBadge(type: string, x: number, y: number) {
  if (type === 'Final') {
    return `
      ...
      <text class="root" x="${x + 29}" y="${y}" font-size="12" font-weight="700" text-anchor="middle" fill="#e84789">Final</text>
    `;
  }
  if (type === 'Initial') {
    return `
      <rect x="${x}" y="${y - 18}" width="64" height="24" rx="4" fill="#eaedfa" />
      <text class="root" x="${x + 32}" y="${y}" font-size="12" font-weight="700" text-anchor="middle" fill="#415cc3">Initial</text>
    `;
  }
  return `<text class="root" x="${x}" y="${y}" font-size="12" fill="#999999">Std</text>`;
}
...
export function buildDetailExportSvg(detail: DetailExportRecord) {
  ...
  const rows = items.map((item, index) => {
    ...
    return `
      ...
      <text class="root" x="${colX.index}" y="${baseline}" font-size="12" fill="#cccccc">${index + 1}</text>
      <text class="root" x="${colX.mark}" y="${baseline}" font-size="17" font-weight="700" fill="#000000">${escapeXml((item.mark || '-').trim() || '-')}</text>
      <text class="root" x="${colX.order}" y="${baseline}" font-size="14" fill="#999999">${escapeXml((item.orderNo || '-').trim() || '-')}</text>
      ${buildTypeBadge(transferType, colX.type, baseline)}
      <text class="root" x="${colX.amount}" y="${baseline}" font-size="17" font-weight="700" text-anchor="end" fill="#000000">$${escapeXml(formatAmount(amount))}</text>
    `;
  }).join('');
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${height}" viewBox="0 0 ${WIDTH} ${height}">
      <defs>
        <style>
          ${fontDataUri ? `@font-face { font-family: 'DetailExportSans'; src: url('${fontDataUri}') format('truetype'); font-weight: 400; font-style: normal; }` : ''}
          .root { font-family: 'DetailExportSans', 'Noto Sans', 'DejaVu Sans', Arial, Helvetica, sans-serif; }
        </style>
      </defs>
      ...
      ${logoDataUri ? `<image href="${logoDataUri}" x="${SIDE_PADDING + 14}" y="${TOP_BORDER + 38}" width="${LOGO_WIDTH}" height="${LOGO_HEIGHT}" preserveAspectRatio="xMinYMid meet" />` : ''}
      <text class="root" x="${WIDTH - SIDE_PADDING - 14}" y="${TOP_BORDER + 58}" font-size="16" font-weight="400" text-anchor="end" fill="#999999">${escapeXml(dateText)}</text>
      ...
      <text class="root" x="${SIDE_PADDING + 14}" y="${TOP_BORDER + HEADER_HEIGHT + 72}" font-size="38" font-weight="700" fill="#415cc3">$${escapeXml(formatAmount(totalAmount))}</text>
      ...
      <text class="root" x="${SIDE_PADDING + sheetWidth / 2 + 14}" y="${TOP_BORDER + HEADER_HEIGHT + 72}" font-size="38" font-weight="700" fill="#415cc3">${items.length}</text>

      <text class="root" x="${colX.index}" y="${tableStartY + 30}" font-size="13" fill="#bbbbbb" letter-spacing="1.2">#</text>
      <text class="root" x="${colX.mark}" y="${tableStartY + 30}" font-size="13" fill="#bbbbbb" letter-spacing="1.2">MARK</text>
      <text class="root" x="${colX.order}" y="${tableStartY + 30}" font-size="13" fill="#bbbbbb" letter-spacing="1.2">ORDER</text>
      <text class="root" x="${colX.type}" y="${tableStartY + 30}" font-size="13" fill="#bbbbbb" letter-spacing="1.2">TYPE</text>
      <text class="root" x="${colX.amount}" y="${tableStartY + 30}" font-size="13" text-anchor="end" fill="#bbbbbb" letter-spacing="1.2">AMOUNT</text>
      ...
      <text class="root" x="${SIDE_PADDING + 14}" y="${footerY + 26}" font-size="12" font-weight="700" fill="rgba(255,255,255,0.65)" letter-spacing="1.2" text-transform="uppercase">${totalLabel}</text>
      <text class="root" x="${WIDTH - SIDE_PADDING - 14}" y="${footerY + 41}" font-size="28" font-weight="700" text-anchor="end" fill="#ffffff">${totalValue}</text>

      <text class="root" x="${SIDE_PADDING + 14}" y="${footerY + FOOTER_HEIGHT + 26}" font-size="12" fill="#cccccc">Mitty Group · Disbursement</text>
      <text class="root" x="${WIDTH - SIDE_PADDING - 14}" y="${footerY + FOOTER_HEIGHT + 26}" font-size="12" text-anchor="end" fill="#cccccc">${escapeXml(`${items.length} records`)}</text>
    </svg>
  `;
}
