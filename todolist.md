...
- NAS-only 完整备份已正式启用：数据库和全部上传文件每天 `02:30` 在 NAS 生成可校验的原子快照，成功后保留 30 天；后台任务通过专用 Docker 容器访问 NAS，失败自动重试并纳入状态巡检
- 历史腾讯云 COS 对象不会删除，但系统不再上传新的云端备份；历史 COS 恢复演练仍作为当时的有效记录保留
...
- 历史重复付款事故 `TRANSFER-1787794481934` 已安全撤销：真实收据和付款明细关联保持不变，错误池订单已清理，正确订单余额恢复为 `$10,453`
- MU Contract 完整对账预览只把同一订单号的多个有效 PI 计为冲突；已失效旧 PI 与有效替代 PI 的历史组合不再误报
...
- `Orders` 页面已接收 MU Contract 正式 PI 的创建日期和金额；人工订单优先，来源订单改名按隐藏 PI 身份跟随，未匹配记录只对管理员可见
- MU Contract Orders 同步已完成首次全量对账并正式启用：53 条来源记录全部匹配，系统按 30 秒间隔增量检查；不会写入发票、收据、付款明细、SWIFT、余额或 NAS 文件
...
- 客户可维护多个通知邮箱和 English/Francais 语言偏好；收款、首次出运和首次放单会生成待审核邮件任务
- 新增仅 ADMIN 可见的 Email Management；客户邮件必须由管理员预览并明确批准，SALES 不能发送，默认禁止外发且默认开启测试收件箱重定向
- 邮件模板、收件人快照、发送尝试和 Resend 回执均保存在业务数据库中；发送中修改会保留修正状态，提前到达的回执可在 Resend 重试时恢复；新增邮件表已通过最新 NAS 快照的两轮隔离恢复验证
- 修复 NAS 备份锁在 SMB 上遗留临时文件导致连续失败的问题；锁改为空目录后，自动备份状态恢复为健康
- 新一轮依赖安全更新已修复网站框架、图片处理和构建工具链的可安全升级项，并从生产镜像移除测试开发工具；业务数据和上传文件不受影响

## 已完成的主要里程碑

### 0. 最新里程碑
- v1.0.217：更新 Next.js、Sharp 及构建工具链安全版本，生产镜像改为锁文件安装并移除开发依赖；剩余 Prisma 配置链公告等待官方兼容修复，不采用风险更高的降级或强制覆盖
- v1.0.216：管理员审核式客户邮件通知完成，覆盖收款、首次出运、首次放单、英法模板、多邮箱、测试模式、发送审计和 Resend 状态回执；数据库迁移与完整媒体已通过隔离恢复演练
- Receipt 余额转移撤销正式上线：迁移和运行服务通过验证，历史重复转移已用管理员事务接口修复，严格审计和重复调用幂等均通过
- Dashboard 客户详情调整为最近收据在历史订单前；未完成签字的收据草稿支持安全修改，收据与签字会话在同一事务中同步，修改后可继续完成签字
- MU Contract -> MULEDGER Orders 已正式上线：53 条 PI 完成首次对账，其中 40 条挂接现有 Orders、13 条创建新 Orders，0 未匹配、0 冲突；增量同步已开启并通过零变更幂等验证
- NAS-only 备份已正式上线：每日 `02:30` 自动备份完整数据库和媒体，保留 30 天；上线前快照通过隔离恢复，上线后快照再次校验通过
- COS 备份完成首次真实恢复演练：从云端实际下载并校验备份，在独立 MariaDB 和临时应用中恢复验证；结果为 `PASS_WITH_FINDINGS`，详细记录已进入备份手册
- 历史实施计划已建立统一状态索引：28 份已完成归档，1 份被最终方案替代，旧计划内未勾选步骤不再视为当前待办
- 已关闭并删除被主线实现取代、且已产生冲突的旧草稿 PR #1
- 2026-07 的依赖安全治理曾将当时公告降至零；2026-09 新公告的可兼容修复已完成，当前只保留 Prisma 配置链的待上游兼容修复项
- 文件路径匹配组件已更新到安全补丁，生产依赖漏洞审计已降至零
