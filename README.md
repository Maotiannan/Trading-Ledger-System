...
- 当前版本：`1.0.100`
- 本次更新：继续修正签名收据模板与手机签字页；`Tel:` 改成固定标签 + 固定高度内容区，电话内容每行最多 14 个字符，只向下覆盖不再推高顶部；中间正文内容重新按原 HTML 模板的标签后紧贴布局对齐，`Motif` 靠左且 `Frais : Paid` 靠右；手机签字页改成黑底 + 居中的窄白签字带，顶部 `Back / Fullscreen / Clear` 与底部 `Complete` 全程可见，无需滚动。
- 前端版本号位置：`设置` 页面最上方
