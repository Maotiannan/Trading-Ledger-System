...
- `Payment Detail Management` 的 `Export Pic` 图片样式已调整：表头改为蓝底白字，日期、序号和底部说明改为蓝色，最终付款和一次性全款改为绿色

## 已完成的主要里程碑

### 0. 最新里程碑
- `Payment Detail Management` 的导出付款明细图片完成视觉样式调整，并新增 `Full payment` 类型用于展示第一笔付款即结清订单的情况
...
### v1.0.174
- `Payment Detail Management` 的 `Export Pic` 图片表头改为蓝底白字，日期、序号和底部说明改为蓝色。
- `Final` 和新增的 `Full payment` 类型改为绿色展示；`Full payment` 表示第一笔付款即结清真实订单。
- `ORDER NO` 列全部加粗，长订单号继续在本列内换行。
