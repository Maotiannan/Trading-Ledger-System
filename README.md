...
- 当前版本：`1.0.168`
- 本次更新：修复 `Generate Signed Receipt` 对复合订单显示错误余额的问题；签名完成后会同步重算对应订单余额，弹窗也会按订单金额减已完成收据实时计算余额。
- 本次更新：`SIGNING_PENDING` 待签名收据不再参与订单余额计算，避免未完成签名的临时收据影响发票、收据和签名收据余额口径。
