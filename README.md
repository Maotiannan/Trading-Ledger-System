...
- 当前版本：`1.0.93`
- 本次更新：`INV 管理` 对 `SALES` 改为整页只读；账单客户解析新增“先精确匹配 `MARK`，再按 `ORDER_NO` 左半部分精确匹配 `ORDER_NAME`”的兜底逻辑；收据管理在直接创建和 OCR 确认创建时，输入 `ORDER` 后会优先从数据库给出 `INV NO` 建议，多命中时自动选最新一条并高亮提醒。
- 前端版本号位置：`设置` 页面最上方
