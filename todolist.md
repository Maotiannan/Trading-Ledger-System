...
- `Create Payment Detail Directly` 可直接勾选 `SR_Received` 收据加入付款明细，同时保留手动录入新行
...
### 0.1 上一里程碑
- `Generate Signed Receipt` 弹窗新增可编辑 `Receipt No.` 字段，默认从最近登记的 10 条收据中取最大数字再加 1
- 用户手动修改 `Receipt No.` 后，本次签名前创建的待签名收据会使用该编号，并推动后续自动编号从它后面继续递增
