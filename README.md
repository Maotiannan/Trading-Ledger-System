...
- 当前版本：`1.0.94`
- 本次更新：收据管理新增“生成签名收据”流程。业务员或管理员可先录入 `ORDER NO + USD Amount`，系统会在签名前原子创建一条 `SIGNING_PENDING` 收据并分配正式 `receiptNo`，然后在桌面新窗口或手机全屏签名页完成双签名。确认后系统会自动生成 PNG、写入 NAS 挂载目录、下载到本地，并把这条收据转为正常业务收据且附上最终图片。
- 前端版本号位置：`设置` 页面最上方
