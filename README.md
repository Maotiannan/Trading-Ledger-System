...
- 当前版本：`1.0.97`
- 本次更新：继续修正签名收据模板导出和移动端签字体验；双签名不再被截断，签名图片改为透明 PNG，长电话自动换行；手机竖屏改为整张收据等比缩放预览，签名完成后的生成流程改为显式 `data:` 转 Blob，避免卡在 `GENERATING...`；`Reçu de M./Mme.` 现在优先显示 `COMPANY_NAME + "MARK"`，为空时回退 `CUSTOMER_NAME + "MARK"`。
- 前端版本号位置：`设置` 页面最上方
