...
export const apiCatalog: ApiModule[] = [
  ...
  },
  {
    ...
    actions: [
      ...
      { action: 'create-session', method: 'POST', description: 'Create a SIGNING_PENDING receipt and signing session', bodyExample: { action: 'create-session', orderNo: 'BIG-ALPHA-07', usdAmount: 2500, paymentMode: 'Espèces', fraisStatus: 'Payé', paymentType: 'Standard' } },
  ...
  },
  {
    ...
    actions: [
      ...
      { action: 'preview-image', method: 'GET', description: 'Serve the currently attached payment detail preview image without browser caching', bodyExample: { detailId: 'detail-id' } },
      { action: 'export-pic', method: 'GET', description: 'Regenerate, attach and download the payment detail export picture', bodyExample: { detailId: 'detail-id' } },
