...
- [x] 依赖安全维护 2026-09：在 12 个新公告包（10 high / 2 moderate；生产 8 个包）基线上，升级 `next / eslint-config-next 16.2.10 -> 16.3.4`、`sharp 0.34.5 -> 0.35.4`、`postcss 8.5.19 -> 8.5.23`，并在现有兼容范围内更新 `nanoid`、三条 `brace-expansion` 及仅开发环境使用的 `@humanfs/node / browserslist / fast-uri / js-yaml`。完整和生产审计均收敛为同一 Prisma CLI 配置链 3 项 high：`prisma 6.19.3 -> @prisma/config 6.19.3 -> deepmerge-ts 7.1.5`；npm 只建议强制降级到 Prisma 6.12.0，当前调用只读取本地可信 Prisma 配置，因此不降级、不强制覆盖不兼容的 `deepmerge-ts 8`，等待 Prisma 官方兼容修复。Docker 构建改为 `npm ci`，构建后 `npm prune --omit=dev`，runner 不再复制完整开发依赖；新增静态契约回归。临时镜像 `muledger-security-test:1.0.216` 验证 Next 16.3.4、Sharp 0.35.4 实际 PNG 生成、Prisma CLI/Client 加载且 Jest 不存在，镜像由约 452 MB 降至 414 MB。完整门禁通过 typecheck、ESLint、Prisma validate/generate、208 suites / 1382 tests、isolated API 24 cases、isolated Playwright 13/13 和 Webpack production build。PR #31 的 GitHub Actions `validate` run `33925150369` 通过后以 merge commit `b31b7fb` 合并；合并后的 `main` 再次通过完整 `test:ci`。v1.0.217 已由安全脚本 app-only 部署，运行镜像确认 Next 16.3.4，28 条迁移无待执行，本地与公网首页均为 200，未登录健康接口按设计为 401，启动日志无错误，NAS `/app/upload` 仍以读写 bind mount 挂载。无 Prisma schema、迁移、业务数据、Docker volume、NAS/COS 路径或备份范围变化。✅ 2026-09-05
...
- [x] Receipt 余额转移撤销与重复付款防护：`BalanceTransfer` 新增对系统生成 `TRANSFER-*` Receipt 的唯一可空关联，迁移只回填双方都唯一的历史候选，歧义数据保持未关联并阻断操作；新增 ADMIN-only 专用撤销入口和共享事务服务，在同一事务内校验关联、删除系统转移收据、还原源订单金额、重算源/目标实时余额、按安全条件清理空系统池订单并写严格审计。管理员直接修改以及 SALES 修改审批统一经过同一个收据修改执行器：首次返回明确确认，确认重试才同时撤销转移并改绑真实 Receipt；过期确认、并发重复、歧义关系和受 Detail/历史/签字等保护引用均停止且不产生部分写入。Receipt 列表隐藏系统转移行的普通修改/删除，USER/SALES 无撤销入口且服务端返回 403；事务内还会同时验证源、目标订单都在管理员层级可见范围，归属变化后不能借可见收据越权修改订单。中英文确认、提交、完成和撤销反馈已统一。事故等价隔离 API 数据验证 `$13,666 - $3,213 = $10,453`，目标只保留真实 Receipt，Detail Item 跟随改绑，错误 `Un_Associated` 源订单清理，重复请求幂等。完整门禁通过 185 suites / 1220 tests、isolated API 22 cases、isolated Playwright 9/9、typecheck、全仓 ESLint、i18n audit 和 Webpack 生产构建；隔离 API 用例各自恢复语言设置，不再依赖执行顺序。新 NAS 快照 `muledger-20260827-181124` 已通过校验并恢复到无生产挂载的 MariaDB 10.6 内存盘，迁移唯一/歧义回填、二次幂等、行数/业务指纹以及事故正式 API 撤销全部通过。PR #26 和合并后 `main` CI 均通过，v1.0.213 已由安全脚本部署，27 条迁移全部生效，HTTPS、维护服务、MU Contract trigger 和 518 个 NAS 文件挂载正常。正式 ADMIN API 已删除 `TRANSFER-1787794481934`、对应 BalanceTransfer 与空错误池 Order；真实 Receipt `0001170` 及其 Detail Item 保持不变，目标缓存/实时余额均为 `$10,453`，严格审计恰好一条，重复调用幂等。新增迁移只改变完整 MySQL `trading_ledger` 快照覆盖范围内的数据，无新 NAS 文件路径。✅ 2026-08-27

- [x] Dashboard 历史区块顺序与待签字收据草稿编辑：共享客户历史组件新增调用方可选的区块顺序，Dashboard 两个客户入口统一显示 `Recent Receipts -> Historical Orders`，Customer Management 保持原顺序。移除 `SIGNING_PENDING` 草稿的直接修改禁令，新增事务内 `receipt-generator-draft-service`，管理员直接修改或销售申请获批时同步更新 Receipt、ORDER/INV 绑定、Detail Item 与待签字会话快照；复用实时订单余额和正式收据布局内核，保留付款类型、付款方式、费用状态与收款人选择，并同步编号、业务日期、客户、付款人、电话、余额和 Motif。缺失会话或签字状态并发变化会明确失败并回滚，旧的“不允许修改”错误文案已删除。验证通过 182 suites / 1188 tests、完整 isolated API 21 cases（含真实草稿修改/恢复/最终签字闭环）、isolated Playwright 9/9、typecheck、全仓 ESLint、i18n audit 和 Webpack 生产构建。无 Prisma schema、迁移、Docker、数据库、NAS 上传路径或备份范围变化；隔离测试容器及临时 volume 已清理，现有服务和业务数据未触碰。✅ 2026-08-26
...
- v1.0.216（2026-09-02）：完成 ADMIN 审核式客户邮件通知。新增客户多邮箱与 English/Francais 偏好、收款/首次出运/首次放单事务投影、六套可维护模板、不可变审批快照、Resend provider adapter、原子发送 claim、稳定幂等键、受控重试与 `Delivery Uncertain`、签名 webhook 去重和独立 Email Management；SALES 仅能在原有可见范围维护客户邮箱，不能访问邮件管理或发送。隔离 API 使用只监听 `127.0.0.1` 的 fake Resend 完成权限、测试重定向、重试与 webhook 闭环，Playwright 覆盖桌面/手机邮箱维护和管理员发送页面；外发默认关闭、测试模式默认开启。完整 NAS 快照在两个 MariaDB 10.6.27 临时库中完成迁移与二次恢复，旧业务指纹保持一致，六张邮件表和 525 个媒体文件全部恢复。同期修复 SMB 备份锁中 `owner.json` 被转换为 `.smbdelete*` 而导致连续失败的问题，锁改为空目录后新快照发布、复验及健康状态均恢复正常。PR 审查后继续收紧并发状态：业务源在邮件发送中发生变化时固定保留 `NEEDS_CORRECTION`，发送完成和 webhook 汇总不得覆盖；有效 webhook 早于 provider message ID 落库时持久化事件并返回可重试响应，同一 `svix-id` 重试后完成关联，不再永久丢失状态。一键安全重建脚本同步刷新邮件发送触发器，避免应用升级后后台发送任务仍停留在旧配置或未启动状态。PR #30 在 required CI 成功后合并为 `36d011c`，合并后的 main CI run `33857744701`、job `100974854469` 再次通过（7m10s）。安全重建以退出码 0 完成，只应用新增邮件迁移，28 条迁移全部为最新；v1.0.216、HTTPS、三个内部触发器及原 NAS 挂载均正常。部署后外发为关闭、测试模式开启，发送器实测返回 disabled 且发送 0 封邮件。✅ 2026-09-04
