...
- 当前版本：`1.0.95`
- 本次更新：修复隔离测试脚本在 macOS 与 Linux 间的临时文件模板兼容性，恢复 GitHub Actions 中 `test:ci` 的跨平台稳定性。收据管理“生成签名收据”流程保持不变，仍支持签名前创建 `SIGNING_PENDING` 收据、原子分配正式 `receiptNo`、双签名后自动生成 PNG、写入 NAS 并挂回收据记录。
- 前端版本号位置：`设置` 页面最上方
