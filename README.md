...
- 当前版本：`1.0.169`
- 本次更新：补齐 `Generate Signed Receipt` 对“完整复合 ORDER NO 精确输入”的余额计算，例如直接输入 `PIKIN-19_B/PIKIN-19B/PIKIN-21` 时也会按真实收据实时计算余额。
- 上一版本：修复 `Generate Signed Receipt` 对复合订单显示错误余额的问题；签名完成后会同步重算对应订单余额，弹窗也会按订单金额减已完成收据实时计算余额。
- 上一版本：`SIGNING_PENDING` 待签名收据不再参与订单余额计算，避免未完成签名的临时收据影响发票、收据和签名收据余额口径。
- 上一版本：新增工程规范：任何新增数据库表、迁移、媒体目录、外部持久化路径或清理任务，都必须同步更新 `muledger` 备份文档、变更检查清单与恢复演练清单。
- 上一版本：新增 `muledger` 腾讯云 COS 备份脚本与运行文档，覆盖 MySQL `trading_ledger` 和 NAS 上传目录。
- 上一版本：已提供本地私有备份配置模板、COSCLI 安装脚本、手动备份脚本和 macOS 每日定时备份安装脚本。
