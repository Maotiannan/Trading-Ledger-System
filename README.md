...
- 当前版本：`1.0.159`
- 本次更新：`Generate Signed Receipt` 的收据号重新从 `0010000` 起按 7 位编号递增，由后端事务内原子分配；弹窗里的编号只作预览，不再允许手动改号，也不再使用“最近 10 条最大编号 +1”的规则。
- 本次更新：`Payment Detail Management` 的小眼睛现在对所有付款明细可用；没有上传原图的明细会自动生成并绑定 `Export Pic` 图片，有上传原图的明细会按“金额 + 日期 + 付款代理”统一命名。
